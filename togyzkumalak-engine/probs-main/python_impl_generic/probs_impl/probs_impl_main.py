...
def go_train_iteration(config: dict, device, model_keeper: helpers.ModelKeeper, evaluage_enemy: helpers.BaseAgent, high_level_i: int, tasks_queues=None, results_queue=None, experience_replay=None):
    ...
    _dbg("H3", "go_train_iteration start", {"iter": high_level_i})
    # #endregion
    ...
    if sub_processes_cnt > 0 and tasks_queues and results_queue:
        ...
        # Serial Q-training if no queues
        for n_games in helpers.split_uniformly(cnt=config['train']['q_train_episodes'], chunks=config['train']['q_dataset_episodes_sub_iter']):
            dataset, stats = probs_impl_train_q.get_dataset(n_games, value_model, self_learning_model, config, device)
            probs_impl_train_q.train_q_model(dataset, value_model, self_learning_model, config, device, self_learning_optimizer)
