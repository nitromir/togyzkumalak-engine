...
def init_battle_worker(env, agents, n_max_steps, randomize_n_turns, verbose):
    global ENV
    global AGENTS_DATA
    global N_MAX_STEPS
...
def multiprocessing_battle(env, agent0, agent1, n_games: int, n_max_steps: int, randomize_n_turns: int, n_threads: int = 0):
    ...
    if n_threads <= 1:
        ...
        def _dbg(hyp, msg, data): open(_log_path, 'a').write(_json.dumps({"hypothesisId": hyp, "location": "battle.py:multiprocessing_battle", "message": msg, "data": data, "timestamp": __import__('time').time()}) + '\n')
        _dbg("H4", "Using mp_context in battle", {"context": mp_context, "platform": sys.platform})
        # #endregion
        
        # Using torch.multiprocessing for better handling of agents
        ...
        with tmp.get_context(mp_context).Pool(
            n_threads,
            initializer=init_battle_worker,
            initargs=(env, [agent0, agent1], n_max_steps, randomize_n_turns, True)
